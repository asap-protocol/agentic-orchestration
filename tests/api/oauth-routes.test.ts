...
describe("OAuth API routes", () => {
  ...
  describe("POST /api/oauth/refresh", () => {
    ...
    it("refreshes access token and preserves refresh token", async () => {
      ...
      refreshAccessTokenMock.mockResolvedValueOnce({ accessToken: "new-access" })
      ...
      expect(refreshAccessTokenMock).toHaveBeenCalledWith("github", "keep-refresh")

      const updated = connectorStore.getConnections().find((c) => c.id === connection.id)
